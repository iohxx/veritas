...
Inference is optional (`VERITAS_INFERENCE_PROVIDER=none` by default). `inference/factory.py` selects adapters locally and imports concrete adapters only when selected. The CLI delegates selection to this module; the pipeline and daemon receive a provider instance and do not import Ollama. No job or Technocore message can change the provider.

LocalInferenceProvider implements the Ollama `/api/chat` API. ExternalInferenceProvider optionally targets a remote Ollama-compatible HTTPS endpoint, selected by local configuration only. It is not a universal adapter for every vendor. Both use InferenceProvider.generate(prompt, context). For a different API, implement that interface inside `inference/`, and wire its locally selected constructor in `inference/factory.py`, or inject the instance into `Pipeline(config, identity, store, provider)`. No change to verification, verdicts, daemon, evidence, signatures or replay is required.

The provider returns the structured contract validated by `verification.semantic.Analysis`: a boolean `full_claim_supported`, a list of findings (source_index, relation, verbatim quote, reason), and a synthesis reason. Invalid or ungrounded output remains insufficient evidence. Arithmetic does not invoke inference. Research without inference still produces signed evidence and can be published; it is not semantically verified. Replay reproduces deterministic checks and verifies saved evidence; it never calls a model. A future FLOP provider can implement the same abstraction when its official API exists; no FLOP protocol is invented now.
